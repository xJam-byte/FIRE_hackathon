"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RoutingController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const routing_service_1 = require("./routing.service");
let RoutingController = class RoutingController {
    routingService;
    constructor(routingService) {
        this.routingService = routingService;
    }
    processAll() {
        return this.routingService.processAllTickets();
    }
    getAssignments(page, limit, businessUnitId, managerId) {
        return this.routingService.getAssignments({
            page,
            limit,
            businessUnitId,
            managerId,
        });
    }
};
exports.RoutingController = RoutingController;
__decorate([
    (0, common_1.Post)('process'),
    (0, swagger_1.ApiOperation)({
        summary: 'Запуск полного пайплайна обработки: AI-анализ → маршрутизация для всех необработанных тикетов',
    }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], RoutingController.prototype, "processAll", null);
__decorate([
    (0, common_1.Get)('assignments'),
    (0, swagger_1.ApiOperation)({ summary: 'Получить все назначения с фильтрами' }),
    (0, swagger_1.ApiQuery)({ name: 'page', required: false, type: Number }),
    (0, swagger_1.ApiQuery)({ name: 'limit', required: false, type: Number }),
    (0, swagger_1.ApiQuery)({ name: 'businessUnitId', required: false, type: Number }),
    (0, swagger_1.ApiQuery)({ name: 'managerId', required: false, type: Number }),
    __param(0, (0, common_1.Query)('page')),
    __param(1, (0, common_1.Query)('limit')),
    __param(2, (0, common_1.Query)('businessUnitId')),
    __param(3, (0, common_1.Query)('managerId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Number, Number, Number]),
    __metadata("design:returntype", void 0)
], RoutingController.prototype, "getAssignments", null);
exports.RoutingController = RoutingController = __decorate([
    (0, swagger_1.ApiTags)('routing'),
    (0, common_1.Controller)('routing'),
    __metadata("design:paramtypes", [routing_service_1.RoutingService])
], RoutingController);
//# sourceMappingURL=routing.controller.js.map