...
${analytics.topProblems.map((p: any) => `  - ${p.type}: ${p.count} обращений (${p.percentage}%)`).join('\n')}

Тональность обращений:
${analytics.tonalityBreakdown.map((t: any) => `  - ${t.tonality}: ${t.count} (${t.percentage}%)`).join('\n')}

Распределение приоритетов:
${analytics.priorityBreakdown.map((p: any) => `  - Приоритет ${p.priority}: ${p.count} обращений`).join('\n')}
...
${analytics.managerLoad
  ...
  .map(
    (m: any) =>
      `  - ${m.fullName}: нагрузка ${m.currentLoad}, назначений ${m.assignmentsCount}`,
  )
  .join('\n')}

Нагрузка офисов:
${analytics.officeLoad.map((o: any) => `  - ${o.name}: ${o.managersCount} менеджеров, ${o.assignmentsCount} назначений, общая нагрузка ${o.totalLoad}`).join('\n')}
...
Ответь ТОЛЬКО JSON, без markdown.`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a senior business analyst. Always respond with valid JSON only, no markdown formatting.',
          },
      ...
      return {
        ...
        aiReport: {
          summary:
            'Не удалось сгенерировать AI-рекомендации. Пожалуйста, попробуйте позже.',
          recommendations: [],
          risks: [],
          kpis: [],
        },
  ...
  private async getTonalityBreakdown() {
    const results = await this.prisma.aiAnalysis.groupBy({
      by: ['tonality'],
      _count: true,
  ...
  private async getCityBreakdown() {
  ...
  private async getRecentTrends() {
    ...
    const avgPriority =
      tickets.length > 0
        ? Math.round(
            (tickets.reduce((s, t) => s + (t.aiAnalysis?.priority || 0), 0) /
    ...
    const negativeCount = tickets.filter(
      (t) => t.aiAnalysis?.tonality === 'Негативный',
    ).length;
    const negativePct =
      tickets.length > 0
